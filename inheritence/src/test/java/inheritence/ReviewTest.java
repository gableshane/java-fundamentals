...
public class ReviewTest{
    ...
    public void testReviewStringMethod(){
        ...
        assertEquals("Testing string output",expected,testReview.toSillyString());
    }
    @Test
    public void testReviewHasRestaurant(){
        Restaurant testaurant = new Restaurant("Testy Restaurant","$");
        Review testReview = new Review("Test review.","Test Author",1);
        testaurant.addReview(testReview);
        String expected = testaurant.name;
        assertEquals("Review should have 'Testy Restaurant as restaurant",expected,testReview.restaurant);
    }
}